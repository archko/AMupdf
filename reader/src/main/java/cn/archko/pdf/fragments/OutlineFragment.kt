...
open class OutlineFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            currentPage = requireArguments().getInt("POSITION", 0)
            if (requireArguments().getSerializable("OUTLINE") != null) {
                outline =
                    requireArguments().getSerializable("OUTLINE") as ArrayList<OutlineItem>
            }

            if (requireArguments().getSerializable("out") != null) {
                outlineItems = requireArguments().getSerializable("out") as ArrayList<OutlineItem>
            }
        }
    ...
    open fun updateSelection(currentPage: Int) {
        ...
        for (i in outline!!.indices) {
            val OutlineItem = outline!![i]
            if (found < 0 && OutlineItem.page >= currentPage) {
                found = i
            }
        }
        if (found >= 0) {
    ...
    protected fun onListItemClick(OutlineItem: OutlineItem) {
        val ac = activity as OutlineListener
        ac.onSelectedOutline(OutlineItem.page)
    }
